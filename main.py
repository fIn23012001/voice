...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Если это видео — извлекаем аудио через MoviePy
        if message.video or (message.document and message.document.mime_type.startswith("video")):
            clip = VideoFileClip(input_path)
            clip.audio.write_audiofile(output_path, codec='libvorbis')
        else:
            # Любой аудиофайл → ogg через ffmpeg
            subprocess.run([
                "ffmpeg", "-i", input_path,
...
if __name__ == '__main__':
    request = HTTPXRequest(connect_timeout=20, read_timeout=20)  # увеличенный таймаут
